...
from methods.lssotd_patched import lssotd_unif
...
def align_loss(x: Tensor, y: Tensor, alpha: float) -> Tensor:
    if alpha == 2.0:
        # Use numerically stable implementation for alpha=2
        return (x - y).pow(2).sum(dim=1).mean()
    else:
        # Add epsilon for numerical stability of norm gradient at 0
        return (x - y).norm(dim=1).add(1e-8).pow(alpha).mean()
...
def pretrain(opt: Options):
    ...
    def lssot_loss(x, y):
        ...
        # Use batch size as ref_size (None means use batch size)
        unif_x = lssotd_unif(x, num_projections=200, ref_size=None, device=x.device, seed=opt.seed)
        unif_y = lssotd_unif(y, num_projections=200, ref_size=None, device=y.device, seed=opt.seed)
        
        unif_loss_val = (unif_x + unif_y) / 2
    ...
    for epoch in range(opt.epochs):
        ...
        for ii, (im_x, im_y) in enumerate(loader):
            ...
            # Check encoder outputs for NaN/Inf - if found, encoder is corrupted
            if torch.isnan(x).any() or torch.isinf(x).any() or torch.isnan(y).any() or torch.isinf(y).any():
                print_stats(f"Warning: NaN/Inf in encoder outputs at epoch {epoch}, iter {ii}. Encoder may be corrupted.")
                # Skip this iteration - encoder outputs are invalid
                it_time_meter.update(time.time() - t0)
                continue
            
            # Check encoder parameters for NaN/Inf
            encoder_has_nan = False
            for param in encoder.parameters():
                if torch.isnan(param).any() or torch.isinf(param).any():
                    encoder_has_nan = True
                    break
            
            if encoder_has_nan:
                print_stats(f"Warning: NaN/Inf in encoder parameters at epoch {epoch}, iter {ii}. Training may be unstable.")
                # Still try to continue, but be very careful

            align_loss_val, unif_loss_val = loss_func(x, y)
            loss = align_loss_val * opt.align_w + unif_loss_val * opt.unif_w

            # Check for NaN/Inf before backpropagation
            is_valid = not (torch.isnan(loss) or torch.isinf(loss) or 
                          torch.isnan(align_loss_val) or torch.isnan(unif_loss_val) or
                          torch.isinf(align_loss_val) or torch.isinf(unif_loss_val))
            ...
            if is_valid:
                ...
                print_stats(f"Warning: NaN/Inf detected at epoch {epoch}, iter {ii}. Skipping backward pass.")
                # Skip optimizer step and meter updates when loss is invalid
                # AverageMeter handles count=0 gracefully (returns 0.0 for avg)
