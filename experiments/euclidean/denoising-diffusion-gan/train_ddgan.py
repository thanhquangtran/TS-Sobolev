...
def train(rank, args):
    ...
    if args.loss == 'cltwd':
        ...
        TWD_obj = torch.compile(TSW(p=2, delta=args.twd_delta, mass_division='distance_based', device=device))
        ...
        pixel_mean = pixel_mean / num_sample
        SbTS_obj = torch.compile(TSSobolev(p=args.ts_sobolev_p, delta=args.twd_delta, mass_division='distance_based', device=device))
