...
import ot
from tree_sliced.tsw import TSW
...
def TWD(X, Y, theta, intercept, mass_division = 'distance_based', p = 2, delta = 2., device = 'cuda'):
    ...
    TWD_obj = TSW(p=p, delta=delta, mass_division=mass_division, device=device)
...
def SbTS(X, Y, theta, intercept, mass_division = 'distance_based', p = 2, delta = 2., device = 'cuda'):
    ...
    L = theta.shape[0]
    nlines = theta.shape[1]
    TWD_obj = TSSobolev(p=p, delta=delta, mass_division=mass_division, device=device)
    return TWD_obj(X, Y, theta, intercept)

def TWD_Chain(X, Y, theta, intercept, subsequent_sources, mass_division = 'distance_based', p = 2, delta = 2., device = 'cuda'):
    L = theta.shape[0]
    nlines = theta.shape[1]
    TWD_obj = TSWChain(nofprojections=L, nlines=nlines, p=p, delta=delta, mass_division=mass_division, device=device)
    return TWD_obj(X, Y, theta, intercept, subsequent_sources)

def SbTS_Chain(X, Y, theta, intercept, subsequent_sources, mass_division = 'distance_based', p = 2, delta = 2., device = 'cuda'):
    L = theta.shape[0]
    nlines = theta.shape[1]
    SbTS_obj = TSSobolevChain(nofprojections=L, nlines=nlines, p=p, delta=delta, mass_division=mass_division, device=device)
    return SbTS_obj(X, Y, theta, intercept, subsequent_sources)
...
def maxTWD(X,Y,n_lines,iterations=50,lr=1e-4, device="cuda"):
    ...
    theta.data/=torch.sqrt(torch.sum((theta.data)**2))
    theta, intercept, subsequent_sources = generate_trees_frames_chain(L = 1, d = dn, theta = theta, range_root_1=-1.0, range_root_2=1.0, range_source_1=-0.1, range_source_2=0.1, nlines=n_lines, device='cuda', type_lines='sequence_of_lines')
