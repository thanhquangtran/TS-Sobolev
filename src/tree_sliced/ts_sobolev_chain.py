...
class TSSobolevChain():
    ...
    def __call__(self, X, Y, theta, intercept, subsequent_sources):
        ...
        # Compute sub_length_target_cumsum for Sobolev weights
        coord_sorted = combined_axis_coordinate_with_intercept_sorted
        sub_length_target_cumsum = coord_sorted - coord_sorted[:, :, 0:1]
        sub_length_right_cumsum = coord_sorted[:, :, -1:] - coord_sorted
        # For chain structure, we need to handle the cumulative length differently
        # We'll use the left cumulative sum for edges before the root, and right for edges after
        ...
        # Reshape to match edge_length
        sub_length_target_cumsum = sub_length_target_cumsum[:, :, :-1]  # Remove last element to match edge_length
        sub_length_target_cumsum = sub_length_target_cumsum.reshape(sub_length_target_cumsum.size(0), -1).unsqueeze(1).clone()
        
        # Compute Sobolev beta edge weight
        if self.p == 2:
        ...
        substract_mass = ((torch.abs(tree_mass)) ** self.p) * beta_edge_weight
...
def generate_trees_frames(L, d, theta = None, range_root_1=-1.0, range_root_2=1.0, range_source_1=-0.1, range_source_2=0.1, nlines=5, device='cuda', type_lines='sequence_of_lines'):
...
if __name__ == "__main__":
    ...
    TW_obj = TSSobolevChain(nofprojections=nofprojections)

    TW_opt_obj = torch.compile(TW_obj, mode="reduce-overhead")
    ...
    theta, intercept, subsequent_sources = generate_trees_frames(nofprojections, dn, nlines=nlines)

    tw = TW_obj(X, Y, theta, intercept, subsequent_sources)

    tw = TW_opt_obj(X, Y, theta, intercept, subsequent_sources)
    ...
    with open("profile_result_compile.txt", "w") as f:

        table_str = prof.key_averages().table(sort_by="self_cuda_memory_usage", top_level_events_only=True)
